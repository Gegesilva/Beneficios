...
switch ($tipoValor) {
    ...
    case $tipoValor == 'U':
        $sql = "SELECT DISTINCT
                        CAST(tb02278_codigo AS VARCHAR(MAX)) AS CODIGO,
                        CAST(TB02278_SITUACAO AS VARCHAR(MAX)) AS SITUACAO,
                        CAST(TB02278_CINTERNO AS VARCHAR(MAX)) AS CODIGO_INTERNO,
                        CAST(TB02278_NUMVENDA AS VARCHAR(MAX)) AS VENDA_ORIGEM_BENEFICIO,
                        CAST(NTFISC AS VARCHAR(MAX)) AS NUM_NOTA,
                        FORMAT(ISNULL (TB02091_DATANOTA, TB02278_DATA), 'dd/MM/yyyy') AS DATA_NOTA,
                        FORMAT(TB02278_DATA, 'dd/MM/yyyy') AS DATA_BENEFICIO,
                        CAST(TB01074_NOME AS VARCHAR(MAX)) AS TIPO_BENEFICIO,
                        CAST(TB02278_NOME AS VARCHAR(MAX)) AS HISTORICO,
                        CAST(TB02278_CODCLI AS VARCHAR(MAX)) AS CODCLI,
                        CAST(A.TB01008_NOME AS VARCHAR(MAX)) AS CLIENTE,
                        CAST(TB02278_MES AS VARCHAR(MAX)) AS MES,
                        FORMAT(TB02278_DTVALIDADE, 'dd/MM/yyyy') AS VALIDADE,
                        TB02278_VLRBENEF AS VALOR_BENEFICIO,
                        TB02278_VLRUTILIZADO AS VALOR_UTILIZADO,
                        TB02278_VLRREST AS VALOR_RESTANTE,
                        CAST(TB02278_MARCANOME AS VARCHAR(MAX)) AS MARCA,
                        CAST(TB01107_NOME AS VARCHAR(MAX)) AS GRUPO_ECONOMICO
        ...
        LEFT JOIN Beneficios ON Beneficios.CODCLI = TB02278_CODCLI AND Beneficios.TIPO_BENEFICIO = TB01074_NOME

        WHERE TB02278_CODCLI = ?
        AND Beneficios.DATA BETWEEN ? AND ?";
        break;
    case $tipoValor == 'E':
        $sql = "SELECT
                    CAST(VW02310.tb02278_codigo AS VARCHAR(MAX)) AS CODIGO,
                    CAST(VW02310.TB02278_SITUACAO AS VARCHAR(MAX)) AS SITUACAO,
                    CAST(VW02310.TB02278_CINTERNO AS VARCHAR(MAX)) AS CODIGO_INTERNO,
                    CAST(VW02310.TB02278_NUMVENDA AS VARCHAR(MAX)) AS VENDA_ORIGEM_BENEFICIO,
                    CAST(TB02021_NTFISC AS VARCHAR(MAX)) AS NUM_NOTA,
                    ...
                    CAST(TB01074_NOME AS VARCHAR(MAX)) AS TIPO_BENEFICIO,
                    CAST(VW02310.TB02278_NOME AS VARCHAR(MAX)) AS HISTORICO,
                    CAST(VW02310.TB02278_CODCLI AS VARCHAR(MAX)) AS CODCLI,
                    CAST(A.TB01008_NOME AS VARCHAR(MAX)) AS CLIENTE,
                    CAST(VW02310.TB02278_MES AS VARCHAR(MAX)) AS MES,
                    ...
                    CAST(VW02310.TB02278_MARCANOME AS VARCHAR(MAX)) AS MARCA,
                    CAST(TB01107_NOME AS VARCHAR(MAX)) AS GRUPO_ECONOMICO,
                    CAST(DATAMES AS VARCHAR(MAX)) AS DATAMES
            ...
            LEFT JOIN TB02091 ON TB02091_NTFISC = TB02021_NTFISC
            LEFT JOIN Beneficios ON Beneficios.CODCLI = VW02310.TB02278_CODCLI AND Beneficios.TIPO_BENEFICIO = TB01074_NOME
